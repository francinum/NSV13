...
#warn DUMMIED OUT - TELESCIENCE OSDRIVER
/*
//telescience driver
	...
	receive_progsignal(var/sendid, var/list/data, var/datum/computer/file/file)
		...
		switch (lowertext(data["command"]))
			if ("set_coords")
				...
				var/datum/computer/file/coords/new_coords = new(src)
...
/datum/computer/file/mainframe_program/driver/nuke
	...
	var/setup_auth_access = ACCESS_ALL_PERSONAL_LOCKERS
	#warn NEED TO ADD ACTUAL DWAINE SU ACCESS (CURRENTLY ACCESS_ALL_PERSONAL_LOCKERS)
	...
	receive_progsignal(var/sendid, var/list/data, var/datum/computer/file/file)
		...
		switch (lowertext(data["command"]))
			...
			if ("arm")
				...
				var/sessionid = "[world.timeofday%100][rand(0,9)]"
				message_device("command=act&auth=[GLOB.netpass_heads]&session=[sessionid]")
			...
			if ("disarm")
				...
				var/sessionid = "[world.timeofday%100][rand(0,9)]"
				message_device("command=deact&auth=[GLOB.netpass_heads]&session=[sessionid]")
...
#warn DUMMIED_OUT (SIGUSR2) Guardbuddy Stuff
/*
