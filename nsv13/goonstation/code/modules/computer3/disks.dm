...
//	icon = 'icons/obj/cloning.dmi'
	...
	item_state = "card-id"
	w_class = 1.0
	//DNA machine vars //HIJACKING THIS FOR NSV
...
/obj/item/disk/data/floppy/examine()
	//set src in oview(5) //This is arbitrary and horribly disgusting -FRANC
	..()
...
/obj/item/disk/data/floppy/read_only
	...
	attack_self(mob/user as mob)
		to_chat(user, "<span style=\"color:red\">You can't flip the write-protect tab, it's held in place with glue or something!</span>")
		return
...
/obj/item/disk/data/floppy/read_only/authentication
	...
	New()
		...
		spawn (10) //Give time to actually generate network passes I guess.
			...
			authrec.fields = list("HEADS"="[GLOB.netpass_heads]",
								"SEC"="[GLOB.netpass_security]",
								"MED"="[GLOB.netpass_medical]")
