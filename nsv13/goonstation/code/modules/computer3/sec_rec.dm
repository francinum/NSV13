...
*/
		...
		src.record_list = GLOB.data_core.general.Copy() //Initial setting of record list.
		...
		switch(menu)
			if (MENU_MAIN)
				switch (command)
					...
					if ("1") //View records
						src.record_list = GLOB.data_core.general
			...
			if (MENU_INDEX)
				...
				src.active_secure = null
				if (GLOB.data_core.general.Find(check))
			...
			if (MENU_FIELD_INPUT)
				...
				switch (field_input)
					if (FIELDNUM_NAME)
						if (ckey(inputText))
							src.active_general.fields["name"] = copytext(inputText, 1, MAX_NAME_LEN)
						else
							return

					if (FIELDNUM_FULLNAME)
						if (ckey(inputText))
							src.active_general.fields["full_name"] = copytext(inputText, 1, MAX_NAME_LEN)
						else
							return
						...
						switch (ckey(inputText))
							if ("y")
								if (src.active_secure)
									...
									GLOB.data_core.security -= src.active_secure
									qdel(src.active_secure)
									src.print_active_record()
									src.menu = MENU_IN_RECORD

								else if (src.active_general)
									GLOB.data_core.general -= src.active_general
		...
		if (!connected)
			if (signal.data["command"] == "ping_reply" && !potential_server_netid)
				...
				if(signal.data["data"] != "noreply")
					var/datum/signal/termsignal = new(src)
	...
	proc
		...
		print_active_record()
			...
			if ((istype(src.active_secure, /datum/data/record) && GLOB.data_core.security.Find(src.active_secure)))
				view_string +={"
		...
		report_arrest(var/perp_name)
			...
			if(src.radiocard.frequency != src.setup_mail_freq && !src.radiocard.setup_freq_locked)
				var/datum/signal/freqsignal = new(src)
		...
		local_print()
			...
			if (istype(src.active_general, /datum/data/record) && GLOB.data_core.general.Find(src.active_general))
				info += {"
