		...
		if(src.net_mode)
			...
			if(signal.data["address_1"] != src.net_id)
				if((signal.data["address_1"] == "ping") && signal.data["sender"])
					var/datum/signal/pingsignal = new(src)
...
*/
			if("print")
				...
				if(!print_data)
					...
					return 1
				spawn(50)
				#warn Make this use thermal paper. For now it's fine being normal paper
					var/obj/item/paper/P = new(src)
					P.forceMove(src.host.loc)
...
/obj/item/peripheral/network/omni
	...
	installed(var/obj/machinery/computer3/newhost)
		...
		if (!src.wireless_link)
			src.wireless_link = SSradio.add_object(src, "[frequency]")
	...
	installed(var/obj/machinery/computer3/newhost)
		...
		spawn(rand(50,100))
			if(host)
				...
				if(host)
					if(prob(50))
						for(var/mob/M in viewers(world.view, host))
							if(M.client)
					...
					for(var/mob/M in viewers(world.view, host))
						...
						new /obj/effect/hotspot(src.host.loc, 0)
						src.host.loc.hotspot_expose(rand(2800,3200), 400)
